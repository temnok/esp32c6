...
func TestTapIdcode(t *testing.T) {
	...
	output := check.Err1(intf.OutEndpoint(2))
	input := check.Err1(intf.InEndpoint(3))
	...
	assert.Equal(t, 22, check.Err1(output.Write(sequence)))

	data := make([]byte, 4)
	assert.Equal(t, 4, check.Err1(input.Read(data)))
